...
new_data_path = r'C:\Users\danii\Desktop\итмо\модель\dataset\кампус\output.xlsx'  # укажи правильный путь
...
# === 6. Добавим факт и год ===
if 'target' in new_df_clean.columns or 'Electricity load (kW)' in new_df_clean.columns:
...
# --- создаём копии почасовых цен для всех годов ---
...
def safe_replace_year(dt, new_year):
    """Безопасная замена года (если 29 февраля, то 28)."""
    try:
...
print(f"Новый диапазон цен: {hourly_prices.index.min()} — {hourly_prices.index.max()}")

allowed_keys = ['cost_cat1_rub', 'cost_cat22_rub', 'cost_cat23_rub', 'cost_cat3_rub', 'cost_cat4_rub']

# === 8. Расчет оптимальной категории по каждому месяцу (БЕЗ ГРАФИКОВ) ===
all_results = []
...
for (year, month) in year_month_pairs:
    ...
    if prices_for_period.empty or prices_for_period.index.nunique() < res_fact.index.nunique() * 0.95:
        print(f"⚠️ Пропускаем {year}-{month:02d}: недостаточно часов в 'Почасовые цены'")
        continue
...
print("\n=== Сравнение по категориям (факт vs прогноз, по месяцам) ===")
print(df_summary)
